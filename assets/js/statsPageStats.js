document.addEventListener("DOMContentLoaded", function () {
  ...
  const statPairs = [["Points","topPoints","moveTopPoints"],["Correct Selections","topCorrect","moveTopCorrect"],["Jokers Won","topJokers","moveTopJokers"],["Points from Jokers","topJokersPoints","moveTopJokersPoints"],["Average points per season","topAvg","moveTopAvg"],["40 Pointers","top40","moveTop40"],["80 Pointers","top80","moveTop80"],["Seasons Entered","topSeasons","moveTopSeasons"]];
  let stats = [], lastStats = [];
  ...
  function updateMovement(currentData,lastData,statField,moveId){
    ...
    if(topCurrent===topLast){ el.textContent="➡️"; el.classList.add("same"); }
    else {
  ...
  function openLeaderboardModal(statField){
    ...
    sortedCurrent.forEach((player,index)=>{
      ...
      tr.innerHTML=`<td>${index+1}</td><td>${player.SportsMaster}</td><td>${value}</td><td>${moveArrow}</td>`;
  ...
  function closeModal(){ modalContent.classList.remove("show"); modalContent.classList.add("hide"); setTimeout(()=>{modal.style.display="none"; modalContent.classList.remove("hide");},300); }
  searchInput.addEventListener("input",()=>{ const filter=searchInput.value.toLowerCase(); tableBody.querySelectorAll("tr").forEach(row=>{ const name=row.children[1].textContent.toLowerCase(); row.style.display=name.includes(filter)?"":"none"; }); });
});
